...
def process_frame(img_bgr,model):
    ...
    # 只要置信度大于0.5的框
    results = model.predict(source=img_bgr,task='detect',show=False,stream=True,device = None,verbose=False,vid_stride=1)
    # vid_stride 视频源的帧间隔默认值是1
...
def run_detect(source):
    ...
    model = YOLO("IH-821-sim.onnx", task='detect')  # 加载模型
    # 日志显示第几号线程的摄像头是否打开成功
    if cap.isOpened():
...
if __name__ == '__main__':
    ...
    ABNORMAL_FRAME_SAVE_DIR = "abnormalFrame"  # 异常帧保存路径
    run_program()
